...
void VulkanEngine::init_background_pipelines()
{
    ...
    {
        fmt::print("Error when building the colored mesh compute shader \n");
    }
    ...
    sky.name = "sky";
    sky.data = {};
    sky.data.data1 = glm::vec4(0.1f, 0.2f, 0.4f, 0.97f); // defauly sky color
    ...
    // We can destroy the shader module directly on the function,
    // we created the pipeline so we have no need for it anymore.
    // With the pipeline and its layout, we need to wait until the end of the program.
...
void VulkanEngine::init_imgui()
{
    ...
    //    but it's copied from imgui demo itself. It won't be a problem, just slightly less efficient space-wise.
    //    ImGui wants its own descriptor pool.
    ...
    init_info.UseDynamicRendering = true; // we won't be using vilkan render-passes but dynamic rendering instead
    // We will draw directly into the swapchain
    ...
    // Immediate submit to upload font texture
...
void VulkanEngine::init_default_data()
{
    ...
    sceneUniformData->metal_rough_factors = glm::vec4{1.0f, 0.5f, 0.0f, 1.0f};
...
void VulkanEngine::init_vulkan()
{
    ...
    //make the vulkan instance, with the basic debug features
    auto inst_ret = builder.set_app_name("George's Example Vulkan Application")
...
{
    // Delete objects in the opposite order they were created in
    if (_isInitialized) {
        ...
        // more like a handle to something that already exists as part of the VkInstance.
        for (int i = 0; i < FRAME_OVERLAP; i++)
        {
    ...
    // Note: VkPhysicalDevice cannot be destroyed, as it’s not a Vulkan resource per-se, it’s more like just a handle to a GPU in the system.
}
...
void VulkanEngine::draw()
{
    ...
    // Wait until the gpu has finished rendering the last frame. Timeout of 1 second and is applied in nanoseconds.
    ...
    VkResult e = vkAcquireNextImageKHR(_device, _swapchain, WAIT_FENCE_TIMEOUT, get_current_frame()._swapchainSemaphore, nullptr, &swapchainImageIndex);
    ...
    // When doing geometry rendering, we need to use color attachment optimal.
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd)
{
    ...
    // 3. Uniform Buffer Allocation using the builder
...
void VulkanEngine::update_scene()
{
    ...
    // Camera view and projection (hard coded for now)
    sceneData.view = glm::translate(glm::vec3{ 0.0f, 0.0f, -5.0f });
...
void VulkanEngine::run()
{
    ...
    while (!bQuit)
    {
        ...
        if (ImGui::Begin("Background"))
        {
            ...
            ImGui::SliderFloat4("data1", reinterpret_cast<float*>(&selected.data.data1), 0, 1.0f);
            ImGui::SliderFloat4("data2", reinterpret_cast<float*>(&selected.data.data2), 0, 1.0f);
            ImGui::SliderFloat4("data3", reinterpret_cast<float*>(&selected.data.data3), 0, 1.0f);
            ImGui::SliderFloat4("data4", reinterpret_cast<float*>(&selected.data.data4), 0, 1.0f);
        }
        ImGui::End();
...
AllocatedBuffer VulkanEngine::create_buffer(size_t allocSize, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage)
{
    ...
    VK_CHECK(vmaCreateBuffer(_allocator, &bufferInfo, &vmaAllocationInfo, &newBuffer.buffer, &newBuffer.allocation, &newBuffer.info));
...
GPUMeshBuffers VulkanEngine::upload_mesh(std::span<uint32_t> indices, std::span<Vertex> vertices)
{
    ...
    // Create vertex buffer (SSBO)
    // VK_BUFFER_USAGE_TRANSFER_DST_BIT : memory copy commands to buffer
    ...
    AllocatedBuffer stagingBuffer = create_buffer(vertexBufferSize + indexBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
    ...
    // Not the most efficient pattern, wait for GPU comand to fully execute before continuiong with CPU side logic.
    // Usually goes on background thread, whose sole job is to execute uploads like this one,
    // and deleting/reusing staging buffers.
...
void VulkanEngine::resize_swapchain()
{
    ...
    // Remake swap chain with updated extent
    int w, h;
